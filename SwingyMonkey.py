...
class SwingyMonkey:
	...
	def get_state(self):
		...
		# return { 'score': self.score,
		#      'tree': { 'dist': math.floor((next_tree['x']+215-self.monkey_right)/40),
		#                'top': math.floor((self.screen_height-next_tree['y'])/40),
		#                'bot': math.floor((self.screen_height-next_tree['y']-self.tree_gap)/40)},
		#      'monkey': { 'vel': math.floor(self.vel/2),
		#                  'top': math.floor((self.screen_height - self.monkey_loc + self.monkey_img.get_height()/2)/40),
		#                  'bot': math.floor((self.screen_height - self.monkey_loc - self.monkey_img.get_height()/2)/40)}}
		return { 'score': self.score,
			'tree': { 'dist': math.floor((next_tree['x']+215-self.monkey_right)/60),
						'top': math.floor((self.screen_height-next_tree['y'])/80),
						'bot': math.floor((self.screen_height-next_tree['y']-self.tree_gap)/80)},
			'monkey': { 'vel': math.floor(self.vel/10),
						'top': math.floor((self.screen_height - self.monkey_loc + self.monkey_img.get_height()/2)/80),
						'bot': math.floor((self.screen_height - self.monkey_loc - self.monkey_img.get_height()/2)/80)}}
	def game_loop(self):
